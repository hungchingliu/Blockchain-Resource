...
contract Arbitrage is IUniswapV2Callee, Ownable {
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external override {
        ...
        // 4. swap WETH to USDC
        if(msg.sender == callbackData.borrowPool) {
    ...
    function arbitrage(address priceLowerPool, address priceHigherPool, uint256 borrowETH) external {
        ...
        IUniswapV2Pair pair1 = IUniswapV2Pair(priceLowerPool); 
        IUniswapV2Pair pair2 = IUniswapV2Pair(priceHigherPool);
        CallbackData memory callbackData;
        callbackData.borrowPool = priceLowerPool;
        callbackData.targetSwapPool = priceHigherPool;
        callbackData.borrowToken = pair1.token0();
        callbackData.debtToken = pair1.token1();
        callbackData.borrowAmount = borrowETH;
        (uint256 reserveOut, uint256 reserveIn, ) = pair1.getReserves();
        callbackData.debtAmount = _getAmountIn(borrowETH, reserveIn, reserveOut); 
        (reserveIn, reserveOut, ) = pair2.getReserves();
        callbackData.debtAmountOut = _getAmountOut(borrowETH, reserveIn, reserveOut);
